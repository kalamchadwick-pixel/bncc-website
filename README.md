# BNCC Website

Landing page sederhana untuk BNCC, dibuat kolaboratif oleh tim 2 orang memakai alur Git & GitHub (branch, Pull Request, code review).

## Visualisasi

![Screenshot](screenshot.png)

Demo: https://kalamchadwick-pixel.github.io/bncc-website

## Tech Stack

- HTML5
- CSS3
- JavaScript (vanilla)
- Git & GitHub

## Fitur Utama

- Navbar dengan anchor link ke tiap section
- Hero section dengan tombol Join Us yang interaktif
- Dark mode / light mode toggle
- Card program yang responsif
- Footer dengan tahun otomatis

## Contribution

| Anggota | Peran | Kontribusi |
|---|---|---|
| Kalam ([@kalamchadwick-pixel](https://github.com/kalamchadwick-pixel)) | Project Initiator & Script Engineer | Membuat repo, mengatur akses tim, menambahkan index.html, menulis README, membuat script.js di branch `feature/scripting` dan menghubungkannya ke index.html |
| Kevin ([@kruvv911](https://github.com/kruvv911)) | Styling Engineer | Membuat style.css di branch `feature/styling` dan menghubungkannya ke index.html |

## What I Learned

Kami belajar membuat branch terpisah, membuka Pull Request, saling mereview, dan menyelesaikan merge conflict di `index.html` ketika dua branch mengubah file yang sama.

## Feature Improvement

- Menambah form kontak
- Menyimpan pilihan dark mode di localStorage
- Animasi scroll antar section
